...
export const FolderCard: React.FC<FolderCardProps> = ({
  ...
  const handleUngroup = async (e: React.MouseEvent) => {
    ...
    setToasts((prev: ToastInfo[]) => [
      ...prev,
      { id: toastId, message: `Ungrouping ${folder.name}...`, type: "loading" },
    ]);
    ...
    try {
      ...
      if (succeeded.length === childrenPaths.length) {
        ...
        setNodes((prev) => {
          ...
          const filteredNodes = prev.filter((n) => {
            if (n.type === "group" && n.id === folder.id) {
              movingLeafs.push(
                ...n.children.map((c) => {
                  const res = succeeded.find(
                    (s) => normalizePath(s.path) === normalizePath(c.path),
                  );
        ...
        setToasts((prev) =>
          prev.map((t) =>
            t.id === toastId ? { ...t, message: "Ungroup success", type: "success" } : t,
          ),
        );
      } else {
        setToasts((prev) =>
          prev.map((t) =>
            t.id === toastId ? { ...t, message: "Partial ungroup failed", type: "error" } : t,
          ),
        );
      }
      setTimeout(
  ...
  const handleDuplicate = async (e: React.MouseEvent) => {
    ...
    setToasts((prev: ToastInfo[]) => [
      ...prev,
      { id: toastId, message: "Duplicating...", type: "loading" },
    ]);
    ...
    try {
      ...
      setToasts((prev) =>
        prev.map((t) =>
          t.id === toastId ? { ...t, message: "Duplicate success", type: "success" } : t,
        ),
      );
    } catch (err: any) {
      setToasts((prev) =>
        prev.map((t) =>
          t.id === toastId
            ? { ...t, message: err.message || "Duplicate failed", type: "error" }
            : t,
        ),
      );
    } finally {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    setToasts((prev: ToastInfo[]) => [
      ...prev,
      { id: toastId, message: "Deleting...", type: "loading" },
    ]);
    ...
    try {
      ...
      setNodes((prev) =>
        prev
          .map((n) => {
            if (n.type === "group")
              return { ...n, children: n.children.filter((c) => c.id !== folder.id) };
            return n;
          })
      ...
      setToasts((prev) =>
        prev.map((t) =>
          t.id === toastId ? { ...t, message: "Delete success", type: "success" } : t,
        ),
      );
    } catch (err: any) {
      setToasts((prev) =>
        prev.map((t) =>
          t.id === toastId ? { ...t, message: err.message || "Delete failed", type: "error" } : t,
        ),
      );
    } finally {
      setIsDeleting(false);
    ...
    >
      {isMultiSelect && !isGroup && (
        <div className="w-10 shrink-0 flex items-center justify-center border-r border-zinc-200 bg-zinc-100/50">
          <div
            className={`w-4 h-4 border-2 transition-all ${isSelected ? "bg-zinc-700 border-zinc-700" : "bg-white border-zinc-300"}`}
          >
            {isSelected && (
      ...
      <div className="flex items-center gap-6 flex-1 min-w-0 p-4 opacity-90">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-4 mb-2">
            <h3
              className={`text-base font-bold truncate ${isBackendConnected ? "text-zinc-800" : "text-zinc-500"} ${isGroup ? "text-zinc-600 italic" : ""}`}
            >
              {folder.name}
              {isGroup && (
                <span className="ml-2 text-[10px] font-black opacity-40 not-italic">[group]</span>
              )}
            </h3>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <div
                className={`flex items-center justify-center w-5 h-5 border transition-colors ${isBackendConnected ? "bg-zinc-700 border-zinc-800" : "bg-zinc-100 border-zinc-200"}`}
              >
                <div
                  className={`w-1 h-1 rounded-full ${isBackendConnected ? "bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.8)]" : "bg-zinc-300"}`}
                />
              </div>
                ...
                <>
                  {!isGroup && (
                    <>
                      <button
                        ...
                        className="px-2 py-0.5 border text-[9px] font-black bg-zinc-200/50 hover:bg-zinc-700 hover:text-white"
                      >
                        Dup
                      </button>
                      <button
                        ...
                        disabled={isDeleting}
                        className="px-2 py-0.5 border text-[9px] font-black bg-zinc-200/50 hover:bg-red-600 hover:text-white"
                      >
                        Del
                      </button>
                    </>
                  ...
                  {isGroup && (
                    <button
                      ...
                      className="px-2 py-0.5 border text-[9px] font-black bg-zinc-700 text-zinc-100 hover:bg-zinc-800"
                    >
                      Ungroup
                    </button>
                  )}
          ...
          <div className="flex items-center gap-4 text-zinc-500">
            <div className="flex items-center gap-1.5 text-[11px] font-medium">
              <span className="truncate max-w-[300px] opacity-70">
                {isGroup ? "Physical Group" : folder.path}
              </span>
            </div>
      ...
      {!isGroup && (
        ...
        >
          <div
            className={`transition-colors ${isMultiSelect ? "text-zinc-100" : ideConfig.value ? "text-zinc-400 group-hover/btn:text-zinc-100" : "text-zinc-300"}`}
          >
            {isMultiSelect ? (
              isSelected ? (
