...
const ManagedFileCard: React.FC<{ file: ManagedFile }> = ({ file }) => {
  ...
  const updateFile = (updates: Partial<ManagedFile>) => {
    setManagedFiles((prev) => prev.map((f) => (f.id === file.id ? { ...f, ...updates } : f)));
  };
  ...
  const handleSync = async () => {
    ...
    try {
      await Promise.all(
        matchedFolders.map((folder) => api.writeFile(folder.path, file.filename, file.content)),
      );
          ...
          <div className="p-4 flex-1 overflow-auto">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] font-black text-zinc-400">Matched folders</span>
              ...
              <div className="space-y-1">
                {matchedFolders.map((folder) => (
                  <div
                    key={folder.id}
                    className="text-[10px] font-mono text-zinc-500 truncate bg-zinc-200/50 px-2 py-1 border border-zinc-200"
                  >
                    {folder.name}
                  </div>
                ))}
      ...
      {managedFiles.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 border border-zinc-300 bg-zinc-50 text-zinc-400">
          ...
          <p className="text-[9px] mt-2 opacity-70">
            Use the toolbar to create a new managed file.
          </p>
        </div>
      ) : (
